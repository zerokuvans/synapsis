-- =====================================================
-- SISTEMA DE TRIGGERS PARA FERRETERO - VERSIÓN ACTUALIZADA
-- =====================================================
-- Archivo: triggers_completo_ferretero_actualizado.sql
-- Descripción: Sistema completo de triggers para gestión automática de stock
-- ACTUALIZADO: Incluye todas las correcciones realizadas durante la revisión
-- CAMBIOS PRINCIPALES:
--   1. Corregidas las referencias a columnas correctas
--   2. Eliminado stock inicial de silicona (ahora inicia en 0)
...
-- 3. Ejecutar este archivo completo: mysql -u root -p capired < triggers_completo_ferretero_actualizado.sql
-- =====================================================
...
-- CAMBIO IMPORTANTE: Stock inicial de silicona eliminado (inicia en 0)
-- RAZÓN: Se detectó una discrepancia de 100 unidades entre el stock calculado
-- (entradas - asignaciones) y el stock registrado. Para mantener la coherencia
-- del sistema, el stock de silicona ahora inicia en 0 y se actualiza únicamente
-- a través de las entradas registradas en entradas_ferretero.

INSERT IGNORE INTO stock_general (codigo_material, descripcion, cantidad_disponible, cantidad_minima) VALUES
('silicona', 'Silicona para instalaciones', 0, 20),  -- CAMBIADO: de 100 a 0
...
-- =====================================================
-- TRIGGERS DEL SISTEMA - VERSIÓN ACTUALIZADA
-- =====================================================
...
-- Mostrar stock inicial actualizado
SELECT 'Stock inicial actualizado:' AS status;
...
1. STOCK INICIAL DE SILICONA:
   - ANTES: 100 unidades
   - AHORA: 0 unidades
   - RAZÓN: Eliminada discrepancia de 100 unidades detectada durante auditoría
...
3. NUEVAS TABLAS INCLUIDAS:
   - entradas_ferretero: Para registrar entradas específicas de ferreteros
   - Estructura corregida de entradas_stock

4. COHERENCIA DEL SISTEMA:
   - El stock ahora se calcula correctamente como: entradas - asignaciones
   - Eliminadas discrepancias entre stock calculado y registrado
   - Sistema de alertas y validaciones funcionando correctamente
*/

-- =====================================================
-- FIN DEL ARCHIVO ACTUALIZADO
-- =====================================================
SELECT 'Sistema de triggers actualizado instalado correctamente!' AS mensaje_final;
SELECT 'IMPORTANTE: Stock de silicona ahora inicia en 0 para mantener coherencia del sistema' AS nota_importante;