...
Versión final que evita problemas con fechas '0000-00-00'
"""
...
def conectar_bd():
    ...
    try:
        connection = mysql.connector.connect(
            ...
            database='capired',
            sql_mode='ALLOW_INVALID_DATES'  # Permitir fechas inválidas
        )
        return connection
...
def verificar_tabla_soat(cursor):
    ...
    cursor.execute("""
        SELECT s.placa_vehiculo, CAST(s.fecha_vencimiento AS CHAR) as fecha_str, r.nombre_completo
        FROM mpa_soat s
...
def verificar_tabla_tecnico(cursor):
    ...
    # Ejemplos de registros válidos
    cursor.execute("""
        SELECT t.placa_vehiculo, CAST(t.fecha_vencimiento AS CHAR) as fecha_str, r.nombre_completo
...
def verificar_tabla_licencias(cursor):
    ...
    # Ejemplos de registros válidos
    cursor.execute("""
        SELECT CAST(l.fecha_vencimiento AS CHAR) as fecha_str, r.nombre_completo
