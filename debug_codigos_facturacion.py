...
def check_server_running() -> List[str]:
    ...
    for base in BASE_URLS:
        ...
        try:
            ...
            log(f"  {base}: status {resp.status_code}")
...
def test_api(base_url: str):
    ...
    try:
        ...
        if resp.status_code == 200:
            try:
                data = resp.json()
                log(f"  ✅ Respuesta JSON con {len(data)} registros")
            except json.JSONDecodeError:
                log("  ❌ La respuesta no es JSON válido")
                log(f"  Body: {resp.text[:300]}...")
        elif resp.status_code in (401, 403):
            log("  ⚠️ No autenticado o sin permisos. Inicia sesión en el navegador y reintenta.")
        elif resp.status_code == 500:
            log("  ❌ Error 500 del servidor. Intentando consultar detalle del error vía DB…")
        else:
